...
class FileSynchronizer
{
    ...
    public function index($media_path)
    {
        $files = $this->filesDetector($media_path);
            ...
            if ($parts == 'mp3') {
                if (!in_array($file, $db)) {
                    ...
                    foreach ($data['tags'] as $information) {
                        $artist['name'] = $information['artist'][0] ?? "Unknown Artist";
                        $artistData = $this->checkArtist($artist);
                        $album['name'] = $information['album'][0] ?? "Unknown Album";
                    ...
                    foreach ($notAvailableFile as $item) {
                        $song = Song::where('path', $item)->delete();
                        Artist::whereNot('name', 'Unknown Artist')->whereNot('name', 'Various Artists')->whereDoesntHave('songs')->delete();
                    }
                }
            }
        }
